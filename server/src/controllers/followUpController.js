...
async function addFollowUp(req, res) {
    ...
    try {
        const newFollowUp = await FollowUp.create({ lead_id: lead_id, user_id: user_id, status_id: status_document._id, comment, date: currentDateTime });
...
async function getFollowUpDate(req, res) {
    try {
        ...
        const filteredFollowUp = Object.keys(groupedFollowUp).reduce((result, leadId) => {
...
// get followup by lead id leatest date by user id
async function getFollowUpDateByUser(req, res) {
    const { user_id } = req.params;

    try {
        const follow_up = await FollowUp.find({ user_id }).populate('status_id').exec();
